...
def configuration_paths() -> list[Path]:
    paths = [*WORKFLOWS.glob("*.yml"), *WORKFLOWS.glob("*.yaml")]
    if ACTIONS.is_dir():
        paths.extend(ACTIONS.rglob("*.yml"))
        paths.extend(ACTIONS.rglob("*.yaml"))
...
def local_reference_exists(reference: str) -> bool:
    ...
    return path.is_dir() and any(
        (path / name).is_file() for name in ("action.yml", "action.yaml")
    )
        ...
        for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            if reference.startswith("./"):
                if not local_reference_exists(reference):
                    errors.append(
                        f"{path.relative_to(ROOT)}:{number}: local action does not exist: "
                        f"{reference}"
                    )
                continue
            if PINNED.fullmatch(reference):
                continue
