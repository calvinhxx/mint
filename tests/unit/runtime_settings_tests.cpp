...
TEST(RuntimeSettings, DefaultsStayWithinHardBounds) {
    ...
    EXPECT_LE(settings.workspace_snapshot_entries,
              mint::runtime_bounds::max_workspace_snapshot_entries);
    EXPECT_LE(settings.workspace_snapshot_bytes,
              mint::runtime_bounds::max_workspace_snapshot_bytes);
    EXPECT_LE(settings.workspace_snapshot_text_bytes, settings.workspace_snapshot_bytes);
...
TEST(RuntimeSettings, ParsesPartialOverridesAndKeepsOtherDefaults) {
    const auto settings = mint::parse_tool_runtime_settings(
        {{"read_file_bytes", 4096},
         {"search_max_hits", 25},
         {"workspace_snapshot_entries", mint::runtime_defaults::workspace_snapshot_entries + 1000},
         {"workspace_snapshot_bytes", mint::runtime_defaults::workspace_snapshot_bytes * 2},
         {"workspace_snapshot_text_bytes",
          mint::runtime_defaults::workspace_snapshot_text_bytes * 2}});
    ...
    EXPECT_GT(settings.workspace_snapshot_entries,
              mint::runtime_defaults::workspace_snapshot_entries);
    EXPECT_GT(settings.workspace_snapshot_bytes, mint::runtime_defaults::workspace_snapshot_bytes);
    EXPECT_GT(settings.workspace_snapshot_text_bytes,
              mint::runtime_defaults::workspace_snapshot_text_bytes);
    EXPECT_EQ(settings.command_resources, mint::CommandResourceLimits{});
}
...
TEST(RuntimeSettings, JsonRoundTripPreservesEveryField) {
    ...
    expected.workspace_snapshot_entries = 5000;
    expected.workspace_snapshot_bytes = 64 * 1024 * 1024;
    expected.workspace_snapshot_text_bytes = 8 * 1024 * 1024;
...
TEST(RuntimeSettings, RejectsUnknownAndOutOfRangeValues) {
    ...
    EXPECT_THROW((void)mint::parse_tool_runtime_settings(
                 ...
                 std::invalid_argument);
    EXPECT_THROW((void)mint::parse_tool_runtime_settings(
                     {{"workspace_snapshot_bytes", 1024}, {"workspace_snapshot_text_bytes", 2048}}),
                 std::invalid_argument);
    EXPECT_THROW(
