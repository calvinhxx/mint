...
TEST(ChangeTransactionTest, AgentRollsBackAndReplaysAnInflightChangeset) {
    ...
    auto control = std::make_shared<mint::TaskControl>();
    {
        ...
        mint::Agent agent(
            model, tools, output, mint::AgentOptions{.max_turns = 1},
            mint::AgentServices{.stop_token = control.get(), .session_repository = &session});
    ...
    try {
        ...
        mint::Agent moved_agent(unused_model, moved_tools, unused_output,
                                mint::AgentOptions{.max_turns = 1, .resume_session = true},
                                mint::AgentServices{.session_repository = &moved_session});
    ...
    mint::Agent resumed_agent(
        ...
        mint::AgentOptions{.max_turns = 1, .resume_session = true},
        mint::AgentServices{.event_sink = &events, .session_repository = &session});
