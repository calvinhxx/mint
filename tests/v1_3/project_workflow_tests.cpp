...
bool contains_string(const aiagent::Json& array, const std::string& expected) {
    if (!array.is_array()) {
        return false;
    }
  ...
  public:
    TemporaryDirectory() {
        ...
        path_ = std::filesystem::temp_directory_path() /
                ("aiagent-v1-3-tests-" + std::to_string(stamp));
...
void expect_private_permissions(const std::filesystem::path& path, const std::string& message) {
    ...
    expect(!error && (permissions & public_permissions) == std::filesystem::perms::none, message);
#else
    (void)path;
    (void)message;
#endif
...
void test_project_detection(const std::filesystem::path& root) {
    ...
    std::filesystem::create_directory_symlink(root / "external-docs", cmake / "docs",
                                              symlink_error);
    expect(!symlink_error, "test setup creates a directory symlink");
#endif

    const auto cmake_suggestion = aiagent::suggest_project_policy(cmake);
    expect(cmake_suggestion.project_kind == "cmake", "CMake project is detected");
    expect(cmake_suggestion.evidence == std::vector<std::string>{"CMakeLists.txt"},
           "CMake detection records evidence");
    expect(contains_string(cmake_suggestion.policy.at("write_paths"), "src") &&
...
#if !defined(_WIN32)
    expect(!contains_string(cmake_suggestion.policy.at("write_paths"), "docs"),
           "policy suggestions do not grant a symlinked write path");
#endif
    expect(cmake_suggestion.policy.at("recipes").size() == 3 &&
    ...
    const auto cargo_suggestion = aiagent::suggest_project_policy(cargo);
    expect(cargo_suggestion.project_kind == "cargo" &&
    ...
    const auto npm_suggestion = aiagent::suggest_project_policy(npm);
    expect(npm_suggestion.project_kind == "npm" &&
    ...
    const auto npm_read_only_suggestion = aiagent::suggest_project_policy(npm_read_only);
    expect(npm_read_only_suggestion.project_kind == "npm-read-only" &&
    ...
    const auto npm_blank_suggestion = aiagent::suggest_project_policy(npm_blank_scripts);
    expect(npm_blank_suggestion.project_kind == "npm-read-only" &&
    ...
    expect(!manifest_symlink_error, "test setup creates a manifest symlink");
    const auto linked_manifest_suggestion = aiagent::suggest_project_policy(linked_manifest);
    expect(linked_manifest_suggestion.project_kind == "generic-read-only" &&
    ...
    const auto generic_suggestion = aiagent::suggest_project_policy(generic);
    expect(generic_suggestion.project_kind == "generic-read-only" &&
...
void test_project_and_task_store(const std::filesystem::path& root) {
    ...
    expect_failure([&] { (void)aiagent::ProjectStore(workspace, workspace / ".state"); },
                   "managed state is rejected inside the workspace");
    expect_failure([&] { (void)aiagent::ProjectStore(workspace, root); },
                   "managed state root cannot be an ancestor of the workspace");

    const auto suggestion = aiagent::suggest_project_policy(workspace);
    aiagent::ProjectStore store(workspace, state);
    expect(!store.initialized(), "new project store starts uninitialized");
    ...
    aiagent::ProjectStore public_store(workspace, public_state);
    ...
    expect((std::filesystem::status(public_state).permissions() &
    ...
    expect(!projects_symlink_error, "test setup creates a projects symlink");
    aiagent::ProjectStore linked_store(workspace, linked_state);
    ...
    expect(store.initialized(), "init persists a project profile and policy");
    expect(store.load_profile().at("project_kind") == "cmake",
           "project profile is bound to the detected project kind");
    expect(store.project_directory().parent_path().parent_path() ==
    ...
    const auto original_policy = aiagent::load_task_policy(store.project_policy_path());
    const auto task = store.create_task("repair the fixture");
    expect(task.directory.parent_path() == store.project_directory() / "tasks",
    ...
    expect(aiagent::load_task_policy(task.policy).fingerprint == original_policy.fingerprint,
           "new task snapshots the current project policy");
    const auto original_metadata = aiagent::SessionStore(task.metadata).load();
    auto mismatched_metadata = original_metadata;
    mismatched_metadata["workspace_root"] = (root / "other-workspace").generic_string();
    aiagent::SessionStore(task.metadata).save(mismatched_metadata);
    expect_failure([&] { (void)store.task_summary(task.id); },
                   "task metadata remains bound to its initialized workspace");
    aiagent::SessionStore(task.metadata).save(original_metadata);
    expect(store.list_tasks().size() == 1 && store.list_tasks().front().status == "created",
           "task is visible before its first agent checkpoint");
    expect(!store.latest_resumable_task().has_value(),
           "task without a checkpoint is not treated as resumable");

    aiagent::SessionStore(task.session)
        .save({{"schema_version", aiagent::session_schema_version},
               {"workspace_root", store.workspace_root().generic_string()},
               {"status", "timed_out"},
               {"verification_status", "not_run"},
               {"turns", std::size_t{3}}});
    const auto summary = store.task_summary(task.id);
    expect(summary.has_value() && summary->status == "timed_out" && summary->resumable &&
               summary->turns == 3,
           "valid interrupted checkpoint becomes resumable");
    const auto resumable = store.latest_resumable_task();
    expect(resumable.has_value() && resumable->id == task.id,
    ...
    expect(aiagent::load_task_policy(store.project_policy_path()).fingerprint !=
    ...
    expect(aiagent::load_task_policy(future_task.policy).fingerprint != original_policy.fingerprint,
           "tasks created after forced init receive the updated policy snapshot");
    expect(store.list_tasks().front().id == future_task.id,
           "task IDs preserve creation order for latest-task discovery");
    const auto still_resumable = store.latest_resumable_task();
    expect(still_resumable.has_value() && still_resumable->id == task.id,
           "latest resumable discovery skips a newer task without a checkpoint");
    const auto demo_task = store.create_task("offline inspection", aiagent::ManagedTaskMode::demo);
    aiagent::SessionStore(demo_task.session)
        .save({{"schema_version", aiagent::session_schema_version},
               {"workspace_root", store.workspace_root().generic_string()},
               {"status", "timed_out"},
    ...
    expect(demo_summary.has_value() && demo_summary->mode == aiagent::ManagedTaskMode::demo &&
               !demo_summary->resumable,
           "interrupted offline demo cannot be resumed as a real model task");

    expect(!store.task_summary("missing-task").has_value(), "missing task query is explicit");
    ...
    aiagent::SessionStore(replacement).save(changed_policy);
    std::filesystem::remove(task.policy);
    std::error_code symlink_error;
    std::filesystem::create_symlink(replacement, task.policy, symlink_error);
    expect(!symlink_error, "test setup replaces task policy with a symlink");
...
int main() {
    try {
        ...
        std::cout << "All v1.3 project workflow tests passed.\n";
        return 0;
