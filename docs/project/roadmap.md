...
新的 `v1.0.0` Release 尚未发布。发布前必须在同一份源码上通过本地 Debug 与 Sanitizer 全量测试、一个真实 provider 的受控握手与修复 fixture，以及六个平台的测试、打包和校验。真实请求只保留脱敏统计，不保存提示词、模型回答、密钥或原始响应。
...
- 正式真实回归只覆盖本次发布选定的 DeepSeek profile；其他 provider 只有离线协议与打包证据；
