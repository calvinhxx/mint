...
RELEASE_URL = "https://github.com/calvinhxx/mint/releases/tag/v{version}"
...
def validate_release_documentation(version: str) -> None:
    expected_url = RELEASE_URL.format(version=version)
    escaped_version = re.escape(version)
    version_marker = rf"(?<![0-9.])v?{escaped_version}(?![0-9.])"
    for relative_path in (
        ...
        if re.search(development, text, re.MULTILINE):
            raise ValueError(f"{relative_path} still marks v{version} as in development")
        if relative_path == "docs/project/roadmap.md" and re.search(
            r"^##[ \t]+发布前剩余[ \t]*$", text, re.MULTILINE
        ):
            raise ValueError("docs/project/roadmap.md still contains a pre-release checklist")
        if f"[`v{version}`]({expected_url})" not in text:
            raise ValueError(f"{relative_path} does not identify v{version} as the current release")
        prerelease = (
            rf"(?:v{escaped_version}.{{0,80}}(?:仍在发布前|尚未发布)|"
            rf"(?:仍在发布前|尚未发布).{{0,80}}v{escaped_version})"
        )
        if re.search(prerelease, text, re.DOTALL):
            raise ValueError(f"{relative_path} still contains pre-release wording")
...
def validate(tag: str | None, require_evidence: bool = False) -> str:
    ...
    if tag is not None:
        ...
        validate_release_documentation(cmake_version)
