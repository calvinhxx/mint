...
std::filesystem::path unique_sibling_path(const std::filesystem::path& target,
    ...
    throw std::runtime_error(message(Message::tools_file_support_temp_path_failed));
}
...
void write_complete_file(const std::filesystem::path& path, std::string_view content) {
    ...
    if (!output) {
        throw std::runtime_error(message(Message::tools_file_support_temp_file_failed));
    }
    output.write(content.data(), static_cast<std::streamsize>(content.size()));
    output.close();
    if (!output) {
        throw std::runtime_error(message(Message::tools_file_support_temp_write_failed));
    }
}
...
void replace_file_safely(const std::filesystem::path& target, std::string_view content,
    ...
    if (target_exists) {
        ...
        if (permission_error) {
            remove_if_present(temporary);
            throw std::runtime_error(message(Message::tools_file_support_permissions_read_failed));
        }
        std::filesystem::permissions(temporary, permissions, std::filesystem::perm_options::replace,
                                     permission_error);
        if (permission_error) {
            remove_if_present(temporary);
            throw std::runtime_error(message(Message::tools_file_support_permissions_copy_failed));
        }
    }
    ...
    if (!target_exists) {
        remove_if_present(temporary);
        throw std::runtime_error(message(Message::tools_file_support_install_failed,
    ...
    if (rename_error) {
        remove_if_present(temporary);
        throw std::runtime_error(message(Message::tools_file_support_stage_failed,
                                         {arg(Placeholder::error, rename_error.message())}));
    }
    ...
    if (rename_error) {
        ...
        remove_if_present(temporary);
        if (rollback_error) {
            throw std::runtime_error(message(Message::tools_file_support_install_restore_failed,
                                             {arg(Placeholder::error, rollback_error.message())}));
        }
        throw std::runtime_error(message(Message::tools_file_support_install_rolled_back,
                                         {arg(Placeholder::error, rename_error.message())}));
    }
...
void remove_file_safely(const std::filesystem::path& target) {
    ...
    if (!std::filesystem::remove(target, error) || error) {
        throw std::runtime_error(message(Message::tools_file_support_remove_failed,
                                         {arg(Placeholder::path, target.generic_string())}));
    }
}
