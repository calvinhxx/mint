...
void add_bytes(std::uintmax_t bytes, std::uintmax_t limit, std::uintmax_t& total,
    ...
    if (bytes > limit - total) {
        throw std::runtime_error("受管工作区快照超过累计字节上限: " + path.string());
    }
    total += bytes;
}
...
WorkspaceEntrySnapshot snapshot_regular_file(const std::filesystem::directory_entry& entry,
    ...
    if (error) {
        throw std::runtime_error("无法读取受管工作区文件大小: " + entry.path().string());
    }
    if (size > limits.max_bytes - total_bytes) {
        throw std::runtime_error("受管工作区快照超过累计字节上限: " + entry.path().string());
    }

    std::ifstream input(entry.path(), std::ios::binary);
    if (!input) {
        throw std::runtime_error("无法读取受管工作区文件: " + entry.path().string());
    }
    std::string contents;
    ...
    if (!input.eof()) {
        throw std::runtime_error("读取受管工作区文件失败: " + entry.path().string());
    }
    if (contents.size() <= runtime_bounds::max_edit_file_bytes && !contains_nul(contents) &&
        ...
        if (contents.size() > limits.max_text_bytes - text_bytes) {
            throw std::runtime_error("受管工作区快照超过文本内存上限: " + entry.path().string());
        }
...
WorkspaceEntrySnapshot snapshot_entry(const std::filesystem::directory_entry& entry,
    ...
    if (std::filesystem::is_symlink(status)) {
        ...
        if (error) {
            throw std::runtime_error("无法读取受管工作区符号链接: " + entry.path().string());
        }
...
WorkspaceSnapshot capture_workspace_snapshot(const std::filesystem::path& root,
    ...
    if (limits.max_entries == 0 ||
        ...
        throw std::invalid_argument("受管工作区快照上限配置无效");
    }
    ...
    if (error) {
        throw std::runtime_error("无法扫描受管工作区: " + root.string());
    }
    ...
    while (iterator != end) {
        ...
        if (!is_valid_utf8(label)) {
            throw std::runtime_error("受管工作区快照包含无法安全表示的路径");
        }
        error.clear();
        const auto status = iterator->symlink_status(error);
        if (error) {
            throw std::runtime_error("无法检查受管工作区路径: " + label);
        }
        if (std::filesystem::is_directory(status) && is_ignored_directory(path)) {
            ...
            if (result.size() == limits.max_entries) {
                throw std::runtime_error("受管工作区快照超过路径数量上限: " + label);
            }
        ...
        if (error) {
            throw std::runtime_error("无法继续扫描受管工作区: " + label);
        }
    }
    return result;
}
