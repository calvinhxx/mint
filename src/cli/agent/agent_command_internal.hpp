...
[[nodiscard]] CommandApproval command_approval(Console& console, bool interaction_jsonl,
                                               EventLog* event_log = nullptr,
                                               std::shared_ptr<TaskControl> task_control = {});
[[nodiscard]] ChangeSetApproval change_set_approval(Console& console, bool interaction_jsonl,
                                                    EventLog* event_log = nullptr,
                                                    std::shared_ptr<TaskControl> task_control = {});
...
tool_options(const CommandLine& command_line, std::vector<std::filesystem::path> protected_files,
             ...
             std::filesystem::path change_transaction_path, EventLog* event_log, Console& console);
