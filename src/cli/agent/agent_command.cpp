...
std::optional<int> prepare_managed_mode(CommandLine& command_line,
    ...
    if (command_line.mode == CommandMode::run) {
        ...
        if (summary->mode == ManagedTaskMode::demo) {
            throw std::runtime_error(message(Message::cli_task_demo_not_resumable));
        }
        if (!summary->resumable) {
            throw std::runtime_error(message(Message::cli_task_status_not_resumable,
                                             {arg(Placeholder::status, summary->status)}));
        }
        ...
        if (!managed_task.has_value()) {
            throw std::runtime_error(message(Message::cli_task_no_resumable));
        }
    }
