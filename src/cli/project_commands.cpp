...
int handle_init_command(const CommandLine& command_line, ProjectStore& store, Console& console) {
    ...
    output << "项目已初始化\n"
           << "类型: " << escape_terminal_field(suggestion.project_kind) << '\n'
           << "工作区: " << escape_terminal_field(store.workspace_root().generic_string()) << '\n'
           << "状态目录: " << escape_terminal_field(store.project_directory().generic_string())
           << '\n'
           << "Policy: " << escape_terminal_field(store.project_policy_path().generic_string())
           << '\n'
           << "检测依据:";
    ...
    output << "\n\n下一步: mint run --root \""
           << escape_terminal_field(store.workspace_root().generic_string()) << "\" \"你的任务\"\n";
    return 0;
}
...
int handle_status_command(const CommandLine& command_line, ProjectStore& store, Console& console) {
    ...
    output << "项目: " << escape_terminal_field(store.workspace_root().generic_string()) << " ("
